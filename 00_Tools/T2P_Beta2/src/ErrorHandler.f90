...
!-------------------------------------------------------------------------------------------------!
  subroutine early_exit()
    ...
    write(*,'(2x,a)') 'Texture2Par Early Termination.'
    call log%write_line('Texture2Par Early Termination')
    call log%close_file()
    error stop
...
!-------------------------------------------------------------------------------------------------!
  ...
  subroutine normal_exit()
    ...
    write(*,*)  ! blank
    write(*,'(2x,a)') 'Texture2Par Normal Termination.'
    call log%write_line('Texture2Par Normal Termination')
    call log%close_file()
    ! TODO write total run time
...
!-------------------------------------------------------------------------------------------------!
  ...
  subroutine error_handler(err,filename,opt_msg)
    ...
    select case(err)
      ...
      case DEFAULT
        write(*,*) "An unknown error has occured. Please contact the developer: lelands@sspa.com"
    end select
