...
!
! 5/10/2024 Leland Scantlebury: accepts a rotational matrix for anistropic
...
contains
  ...
  function kdtree2_create(input_data,success,dim,sort,rearrange,rotmat) result (mr)
    !
    ...
    ! LS: Added non-optional `success` argument to determine if tree was
    !     created successfully. Added failure for >=1 input_data values.
    ...
    integer, intent(inout) :: success   ! 1 is success, 0 is failure
    !
    integer :: i
    ! ..
    allocate (mr)
    ...
    if (success==0) return    ! EARLY RETURN
...
!!$
      ...
      ! DEBUG
      !write(*,*) "build_tree_for_range: l=",l,"  u=",u
      ...
      if ((u-l)<=bucket_size) then
         ...
         ! DEBUG
         !if (l==15466.AND.(u==15485.OR.u==15465)) then
         !  do j = l, u
         !    vals(j-l+1) = tp%the_data(c, tp%ind(j))
         !  end do
         !end if
...
!         res%cut_val = tp%the_data(c,tp%ind(m))
  ...
  subroutine kdtree2_n_nearest(tp,qv,nn,results,rotmat,mask)
    ...
    sr%rotmat => tp%rotmat
    ! Overwrite rotmat if new one passed
    if(present(rotmat)) then
      sr%dist_type = 1
      sr%rotmat => rotmat
    end if
    ...
    !else
      !sr%mask => tp%mask
    !end if
    !if (tp%rearrange) sr%mask = sr%mask(sr%ind)
    ...
    if (present(mask)) then
      if (tp%rearrange) then
        !sr%mask = sr%mask(sr%ind) ! temporary array causes stack overflow when big data
        n_points = size(sr%ind)

        if(associated(sr%mask)) deallocate(sr%mask)
        allocate(sr%mask(n_points))
        do i = 1, n_points
            sr%mask(i) = mask(sr%ind(i))
        end do
